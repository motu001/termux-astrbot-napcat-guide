...
- **2026-09-25 单次真实重启检查**：开机脚本触发、核心管理服务与 SSH 恢复；最初 QQ 未自动登录，真实 OneBot 连接为 0。机主随后手动点击登录，状态接口显示 QQ 在线，AstrBot 真实适配器连接日志出现 1 次连接。**这不是 QQ 无人值守自动登录的证明**。仍未证明：全新手机从本文完整安装一次、此次重启后的另号逐消息收发、跨多次重启稳定恢复及可归因到 AI 模型生成的回复。因此本文不是“复制命令即可保证成功”的承诺。
